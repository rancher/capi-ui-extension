...
export const makeSchemas = function(openSchema, id = 'ccvariable') {
  ...
  const properties = openSchema.type === 'array' ? openSchema?.items?.properties : openSchema?.properties || {};
  ...
  for (const key in properties) {
    ...
    if (SIMPLE_TYPES.includes(def.type)) {
      ...
      console.log('array type schema: ', def);
...
export const makeYamlPlaceholders = function(openSchema) {
